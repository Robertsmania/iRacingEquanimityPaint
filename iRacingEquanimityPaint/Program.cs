...
using HerboldRacing;  // Replace with the actual namespace where IRSDKSharper is located, if different.

namespace iRacingEquanimityPaint
{
    class Program
    {
        // Class-level field accessible throughout the Program class
        static IRSDKSharper irsdk = new IRSDKSharper();
        static int subSessionID = 0;
        ...
        // This method is invoked when the IRSDKSharper fires the OnSessionInfo event
        static void OnSessionInfo()
        {
            var thisSubSessionID = irsdk.Data.SessionInfo.WeekendInfo.SubSessionID;
            var trackName = irsdk.Data.SessionInfo.WeekendInfo.TrackName;
            var driverInfo = irsdk.Data.SessionInfo.DriverInfo;
            ...
            Console.WriteLine($"SubSessionID: {thisSubSessionID} Track: {trackName} Drivers: {driverInfo.Drivers.Count} ");
        }
        static void OnSessionInfoX()
        {
            // Implementation depends on how you want to handle the session information updates
            // You might want to process session info and update liveries here
            Console.WriteLine("Session Info Updated");
        }

        // Asynchronously wait for the user to press 'Q' to quit, using the passed CancellationToken
        static async Task MonitorQuitAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
